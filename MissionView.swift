...
struct MissionView: View {
    ...
    private var missionInfoSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                InfoRow(icon: "clock", color: .gray, text: "任务每日重置，24小时冷却")
                InfoRow(icon: "arrow.up.circle", color: .purple, text: "概率提升最高可达100%")
            }
        }
        .padding()
        .background(Color(.systemBackground))
    ...
    // MARK: - Actions
    private func completeMission(_ mission: MissionType) {
        if gameManager.completeMission(mission) {
            completedMission = mission
            alertMessage = "完成「\(mission.name)」\n获得 +\(Int(mission.probabilityBoost * 100))% 概率提升!"
            showingAlert = true
        }
    }
}
